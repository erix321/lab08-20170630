			...
			if(uSearch.isEmpty()){
				...
				if(rSearch.isEmpty()){
					...
					if(aSearch.isEmpty()){
						...
						Long lng=(Long) request.getAttribute("userId");
						if(lng==null){
							lng=new Long(request.getParameter("userId"));
						}
						Key k=KeyFactory.createKey("User",new Long(lng));
						User u = pm.getObjectById(User.class, k);
						request.setAttribute("user", u);
						k=KeyFactory.createKey("Role",u.getIdRole());
